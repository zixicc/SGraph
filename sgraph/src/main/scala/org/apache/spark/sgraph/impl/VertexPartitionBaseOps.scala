 ...
 */
private[sgraph] abstract class VertexPartitionBaseOps
  ...
  (other: UpdateVertexPartition[VD2])
  (f: (VertexId, VD, Option[VD2]) => VD3): Self[VD3] = {
    if (self.index != other.index) {
      logWarning("left join -- Joining two VertexPartitions with different indexes is slow.")
  ...
  (other: UpdateVertexPartition[U])
  (f: (VertexId, VD, U) => VD2): Self[VD2] = {
    if (self.index != other.index) {
      logWarning("inner join -- Joining two VertexPartitions with different indexes is slow.")
  ...
  : UpdateVertexPartition[VD2] = {
    ...
    new UpdateVertexPartition[VD2](self.index, newValues, newMask)
  }
