...
import org.apache.spark.sgraph.impl.RoutingTablePartition.RoutingTableMessage
...
class RoutingTableMessageRDDFunctions(self: RDD[RoutingTableMessage]) {
  ...
  def copartitionWithVertices(partitioner: Partitioner): RDD[RoutingTableMessage] = {
    new ShuffledRDD[VertexId, Int, (VertexId, Int)](
      self, partitioner).setSerializer(classOf[RoutingTableMessageSerializer].getName)
  }
}
...
object RoutingTablePartition {
  ...
  def edgePartitionToMsgs(pid: PartitionID, edgePartition: EdgePartition[_])
    ...
    edgePartition.sourceIndex.array.iterator.foreach { e =>
      map.changeValue(e._1, 0x1, (b: Byte) => (b | 0x1).toByte)
    }
