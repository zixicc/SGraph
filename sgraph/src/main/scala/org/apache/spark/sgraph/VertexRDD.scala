...
import org.apache.spark.sgraph.util.SGraphHashPartitioner
...
import org.apache.spark.sgraph.impl.{VertexAttributeBlock, MsgRDDFunctions, VertexPartition, RoutingTablePartition}
import org.apache.spark.sgraph.impl.RoutingTableMessageRDDFunctions._
 ...
 * `VertexRDD`, use the [[org.apache.spark.sgraph.VertexRDD$ VertexRDD object]].
 *
...
class VertexRDD[@specialized VD: ClassTag](
   ...
   */
  def reindex(): VertexRDD[VD] = new VertexRDD[VD](partitionsRDD.map(_.reindex()))
  ...
  override def cache(): VertexRDD[VD] = {
   ...
   */
  private[sgraph] def mapVertexPartitions[VD2: ClassTag](f: VertexPartition[VD] => VertexPartition[VD2])
    : VertexRDD[VD2] = {
    val newPartitionsRDD = partitionsRDD.mapPartitions(_.map(f), preservesPartitioning = true)
    new VertexRDD(newPartitionsRDD, targetStorageLevel)
  }
  ...
  /**
   * Left joins this RDD with another UpdateVertexRDD with the same index. This function will fail if both
   ...
   *
   * @tparam VD2 the attribute type of the other UpdateVertexRDD
  ...
  def leftZipJoin[VD2: ClassTag, VD3: ClassTag]
      (other: UpdateVertexRDD[VD2])(f: (VertexId, VD, Option[VD2]) => VD3): VertexRDD[VD3] = {
    ...
    new VertexRDD[VD3](newPartitionsRDD, targetStorageLevel)
  }
  ...
  def leftJoin[VD2: ClassTag, VD3: ClassTag]
    ...
    other match {
      case other: UpdateVertexRDD[VD2] =>
        leftZipJoin(other)(f)
      case _ =>
        new VertexRDD[VD3](
  ...
  def aggregateUsingIndex[VD2: ClassTag](
    ...
    val shuffled = MsgRDDFunctions.partitionForShuffling(messages, this.partitioner.get)
    ...
    new VertexRDD[VD2](parts, targetStorageLevel)
  }

  def shipDestinationVertices(): RDD[(PartitionID, VertexAttributeBlock[VD])] = {
    this.partitionsRDD.mapPartitions(_.flatMap(_.shipDestinationVertices()))
  }

  def shipDestinationVertices[A: ClassTag](activeVertexSet: UpdateVertexRDD[Null]): RDD[(PartitionID, VertexAttributeBlock[VD])] = {
    this.partitionsRDD.mapPartitions(_.flatMap(_.shipDestinationVertices()))
  }
   ...
   * [[org.apache.spark.sgraph.VertexRDD#cache]] on the returned VertexRDD.
   */
...
object VertexRDD {
  ...
  def apply[VD: ClassTag](rdd: RDD[(VertexId, VD)]): VertexRDD[VD] = {
    val partitioned: RDD[(VertexId, VD)] = rdd.partitioner match {
      case Some(p) => rdd
      case None => rdd.partitionBy(new SGraphHashPartitioner(rdd.partitions.size))
    }
  ...
  def fromEdges[VD: ClassTag](
    ...
    val routingTables = createRoutingTables(edges, new SGraphHashPartitioner(numPartitions))
    val vertexPartitions = routingTables.mapPartitions({ routingTableIter =>
      val routingTable =
        if (routingTableIter.hasNext) routingTableIter.next() else RoutingTablePartition.empty
      Iterator(VertexPartition(Iterator.empty, routingTable, defaultVal))
    }, preservesPartitioning = true)
    new VertexRDD(vertexPartitions)
  }
