...
object SparkBuild extends Build {
  ...
  lazy val repl = Project("repl", file("repl"), settings = replSettings)
    .dependsOn(core, graphx, sgraph, bagel, mllib)
  ...
  lazy val sgraph = Project("sgraph", file("sgraph"), settings = sgraphSettings) dependsOn(core)
  ...
  lazy val assemblyProj = Project("assembly", file("assembly"), settings = assemblyProjSettings)
    .dependsOn(core, graphx, sgraph, bagel, mllib, repl, streaming) dependsOn(maybeYarn: _*) dependsOn(maybeGanglia: _*)
  ...
  lazy val examples = Project("examples", file("examples"), settings = examplesSettings)
    .dependsOn(core, mllib, graphx, sgraph, bagel, streaming, externalTwitter) dependsOn(allExternal: _*)

  // Everything except assembly, tools and examples belong to packageProjects
  lazy val packageProjects = Seq[ProjectReference](core, repl, bagel, streaming, mllib, graphx, sgraph) ++ maybeYarnRef ++ maybeGangliaRef
 ...
 def replSettings = sharedSettings ++ Seq(
  ...
  def sgraphSettings = sharedSettings ++ Seq(
    name := "spark-sgraph",
    libraryDependencies ++= Seq(
      "org.apache.commons" % "commons-math3" % "3.2"
    )
